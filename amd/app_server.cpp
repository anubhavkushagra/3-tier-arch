...
// RocksDB will be hosted on the AMD laptop
const std::string INTERNAL_STORAGE_IP = "192.168.0.211:50005";
...
public:
    void Run() {
        // --- 1. CONNECT TO INTERNAL STORAGE SERVER (i5) ---
        ...
        std::cout << ">>> PUBLIC APP SERVER (TLS/JWT) Live on 0.0.0.0:50051 (AMD: 192.168.0.211) <<<" << std::endl;
    ...
    struct CallData {
        ...
        void Proceed() {
            if (status == CREATE) {
                ...
                // In a real app we'd verify the JWT properly here.
                // We'll delegate that to our previous implementation or dummy check for now
